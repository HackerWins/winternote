...
import Dom from './Dom'
import { debounce, isFunction, spread } from '../util/func';
...
export default class EventMachin {
  ...
  parseComponent () {
    ...
    Object.keys(this.childComponents).forEach(ComponentName => {
      ...
      spread(targets).forEach($dom => {
        let props = {};
        
        spread($dom.el.attributes).filter(t => {
  ...
  parseEvent (key) {
    ...
    eventNames.forEach(eventName => {
      var eventInfo: any[] = [eventName, ...params]
      // console.log(eventInfo)
      this.bindingEvent(eventInfo, checkMethodFilters, callback);
    })
  }

  getDefaultDomElement (dom): any {
    let el; 
  ...
  bindingEvent ([ eventName, dom, ...delegate]: any[], checkMethodFilters, callback) {
    let eventObject: any = this.getDefaultEventObject(eventName, checkMethodFilters);
